...
if __name__ == '__main__':
    ...
    if args.months:
        for tier in args.format:
            print '========== ' + tier + ' =========='
            data1, info1 = util.readFile(args.months[0], tier, listify=False)
            data2, info2 = util.readFile(args.months[1], tier, listify=False)
            x = {k: util.usageProportion(v, info1) for k, v in data1.items()}
            y = {k: util.usageProportion(v, info2) for k, v in data2.items()}
        ...
        for tier in args.format:
            ...
            for i in range(len(all_months)):
                if i == 0:
                    continue
                data1, info1 = util.readFile(all_months[i-1], tier, listify=False)
                data2, info2 = util.readFile(all_months[i], tier, listify=False)
                x = {k: util.usageProportion(v, info1) for k, v in data1.items()}
                y = {k: util.usageProportion(v, info2) for k, v in data2.items()}
